...
(deftype float8() 'doubble)


(defclass gop-type () 
"""symbol-of := symbol associated with type,
ofType: defined type for validation"
...
;save time by seeing if the type[s] involved require conversion or not. 
;important mainly for collections when going from one context such as cache to
;another such as client.  
(defmethod needs-coercion? ((ctype gop-type)) nil)
(defmethod needs-coercion? ((type general-object) t)
(defmethod needs-coercion? ((type collection-type)) (needs-conversion? (contentType type)))

(defmethod cache-coerce ((type gop-type) value) value)

(defmethod cache-coerce ((type general-object) value)
  (as-loid value)

(defun get-cache-oercion ((type gop-type))
  (lambda (val) (cache-coerce type val)))


(defmethod map-over (some-collection aFuntion)
  (copy-applying aFuntion some-collection)
  
(defmethod cache-coerce ((type collecton-type) values) 
  (let ((result (make-instance (type-of values))))
    ;assuming values is map-able? check/insure
    ;no. need to learn about generating streams I think
    (map-over values  #'(lambda (val) (add result (cache-coerce result val))))
    result))

(defmethod cache-coerce ((type map-type) map)
  (let ((result (make-instance (type-of map))))
    (labels (
             (key-convert (val) (cache-coerce (key-type type) val))
             (value-convert (val) (cache-coerce (value-type type) val)))
    (maphash #'(lambda (key val) 
                 (setf (gethash (key-convert key) result)(value-convert val)))
             map)
    result)))